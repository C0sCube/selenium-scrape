...
import re, hashlib, inspect, dateutil, ocrmypdf, pandas as pd
...
class OperationExecutor:
    ...
    def _generalize_table_df(self, html_str) -> str:
        ...
        # 1️⃣ Guard against invalid or empty HTML
        if not isinstance(html_str, str) or "<table" not in html_str.lower():
            self.logger.warning("normalize_df skipped: Input not valid table HTML.")
            return "[NO TABLE FOUND]"

        # 2️⃣ Try reading tables safely
        try:
            ...
            self.logger.error(f"Unexpected error in normalize_df: {e}")
            return "[FAILED TO PARSE TABLE]"

        if not dfs:
            self.logger.warning("normalize_df found no tables — skipping.")
            return "[EMPTY TABLE SKIPPED]"

        # 3️⃣ Normalize non-empty tables
        all_dfs = []
        for df in dfs:
            if df.empty:
                continue
        ...
        if not all_dfs:
            self.logger.warning("normalize_df: all tables were empty after cleaning.")
            return "[EMPTY TABLE SKIPPED]"

        final_df = pd.concat(all_dfs, ignore_index=True)
        norm_df = final_df.to_csv(index=False, header=False, sep='|', lineterminator='\n')

        return norm_df if norm_df.strip() else "[EMPTY TABLE SKIPPED]"
    ...
    def _parse_table(self, entry):
        """
        Parse scraped entry into a JSON-safe structure (list of lists),
        ensuring no raw DataFrames are returned.
        """
        import pandas as pd
        from bs4 import BeautifulSoup
        import base64, pdfplumber, tempfile
        ...
        try:
            if content_type == "table_html":
                ...
                return [["text"], *[[t] for t in text_lines]]
                ...
                try:
                    ...
                    return [[f"Invalid PDF file: {e}"]]
                return all_rows or [["No table found in PDF"]]
                ...
                try:
                    ...
                    return [[f"OCR PDF failed: {e}"]]
                return all_rows or [["No table found in OCR PDF"]]
            ...
            if hasattr(self, "logger") and self.logger:
                ...
                print("[ERROR]", msg)
            return [[f"Failed to parse data of type: {content_type}"]]
...
import time ,re,os, hashlib, inspect, dateutil, base64, pdfplumber, ocrmypdf, tempfile
import pandas as pd
from bs4 import BeautifulSoup
from dateutil.parser import parse
from io import StringIO, BytesIO
...
class OperationExecutorLatest:
    ...
    def generate_sorted_excel_report(self, comparison_json, output_path="DepositRate_Comparison_Report.xlsx"):
        ...
        RED_NOTE_FILL = PatternFill(start_color="FFCCCC", end_color="FFCCCC", fill_type="solid")
        HEADER_FILL = PatternFill(start_color="E0E0E0", end_color="E0E0E0", fill_type="solid")
        ...
        with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
            ...
            change_col = summary_headers.index("Change %") + 1
            change_range = f"{ws_summary.cell(row=2, column=change_col).coordinate}:{ws_summary.cell(row=len(summary_df)+1, column=change_col).coordinate}"
            ws_summary.conditional_formatting.add(change_range, CellIsRule(operator="greaterThanOrEqual", formula=["50"], fill=RED_FILL))
            ws_summary.conditional_formatting.add(change_range, CellIsRule(operator="between", formula=["20", "49.99"], fill=YELLOW_FILL))
            ws_summary.conditional_formatting.add(change_range, CellIsRule(operator="lessThan", formula=["20"], fill=GREEN_FILL))

            note_col = summary_headers.index("Notes") + 1
            note_range = f"{ws_summary.cell(row=2, column=note_col).coordinate}:{ws_summary.cell(row=len(summary_df)+1, column=note_col).coordinate}"
            ws_summary.conditional_formatting.add(note_range, FormulaRule(formula=[f'LEN({ws_summary.cell(row=2, column=note_col).coordinate})>0'], fill=RED_NOTE_FILL))
            ...
            for record in sorted_records:
                ...
                for i in range(max_tables):
                    ...
                    # elif new_entry:
                    #     new_df = self._parse_table(new_entry)
                    #     row_cursor = self._write_single_table(ws, new_df, start_row=row_cursor, title=title + ["⚠️ No old data available"])
                    # elif removed_entry:
                    #     removed_df = self._parse_table(removed_entry)
                    #     row_cursor = self._write_single_table(ws, removed_df, start_row=row_cursor, title=title + ["⚠️ No new data available"])
                    # else:
                    #     ws.cell(row=row_cursor, column=1, value="⚠️ No data available for comparison")
                    #     row_cursor += 2
